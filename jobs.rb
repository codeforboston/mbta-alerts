...
class AlertTweeter
  ...
  def self.perform
    ...
    untweeted_ids.each do |id|
      puts "-----#{id}----"
      alert_message = Alert.redis.hgetall("alerts:#{id}")['message']
      tweetable_alert = TweetHelpers.tweetify alert_message
      puts "TWEETABLE: #{tweetable_alert}"
      Alert.redis.srem "untweeted:#{id}"
      puts "removed"
